...
// by T-001-17's real F-001 auth screens, restructured by D-023's mobile
// architecture refactor). Confirms the app boots and renders the login
// screen by default (after the cold-start bootstrap restore finds no
// session, T-001-17 M-2) — proves the generated Dart client
// (package:omnistock_api_client) is a real, wired, compile-time dependency
// exercised through lib/features/auth/** (AuthApi/LoginRequest/
// TokenResponse/...), not just a single unused enum reference as in the
// original F-000 stub.
...
import '../features/auth/data/fakes.dart';
...
void main() {
  testWidgets('OmniStock boots through the bootstrap gate to the login screen (no keychain session)', (
    ...
    final authClient = AuthRepositoryImpl(
    ...
    await tester.pumpWidget(
      ProviderScope(
        overrides: [authRepositoryProvider.overrideWithValue(authClient)],
