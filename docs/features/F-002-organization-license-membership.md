...
> US-5 เพิ่ม "ออกจากร้านเอง" ตาม **D-029** ·
> **แก้รอบ Gate E (2026-09-20):** US-3 **เติม AC "เชิญใหม่อีกครั้ง" ย้อนหลัง** (N-3 — scope ที่ลงมาหลัง Gate 2 โดยไม่มี AC) ⇒ **AC 35 → 36** · เหตุผล: [`F-002/release-gate-f.md §7.2`](F-002/release-gate-f.md) · **UI ใช้คำว่า "ร้าน" แทน "องค์กร"** (D-029 — code/API ยังเป็น `organization`)
...
- **US-3:** ในฐานะ Owner/Admin ฉันต้องการ **เชิญสมาชิก**
  ...
  - [ ] AC (**เติมย้อนหลัง 2026-09-20 · N-3** — ดูกล่องใต้ US-3): คำเชิญที่ **หมดอายุ** หรือ **ถูกยกเลิก** ต้องเชิญซ้ำได้จากแถวนั้นเลย
        ด้วยปุ่ม **"เชิญใหม่อีกครั้ง"** · ⛔ **ห้ามแสดงปุ่มนี้บนแถวที่ `accepted`** (คนนั้นอยู่ในร้านแล้ว — "เชิญใหม่" เป็นคำที่ไม่จริง
        และพาไปผิดทาง · ถ้าเขาถูกถอดทีหลัง ทางที่ถูกคือปุ่ม "เชิญสมาชิก" ตามปกติ) · กดแล้วเปิดฟอร์มเชิญโดยเติม **email + สิทธิ์เดิม**
        เป็น**ค่าตั้งต้นที่ยังแก้ได้** · (**D-028**) ถ้าแถวเดิมเป็น `Owner` แต่คนกดไม่ใช่ Owner → **ไม่เลือกสิทธิ์ไว้เลย**
        (ห้าม pre-fill ตัวเลือกที่กดส่งไม่ผ่าน) · **ผลลัพธ์คือคำเชิญ*ใบใหม่* ไม่ใช่การหมุน token ของใบเดิม**
        ⇒ ไม่ใช่ "ออกลิงก์ใหม่" ของ AC ข้างบน และไม่ต้องยืนยันแบบเดียวกัน · ข้อมูลเก่า (เข้าร้านไปแล้ว/มีคำเชิญค้าง)
        ให้ server ปฏิเสธและใช้ข้อความของฟอร์มเชิญที่มีอยู่แล้ว — **ไม่ขอ field ใหม่**

> **⚠️ AC ข้อสุดท้ายของ US-3 ถูกเติมย้อนหลังหลัง Gate 2 — บันทึกที่มาไว้เพื่อไม่ให้ตัวเลขกลายเป็นคำโกหก**
> ปุ่ม "เชิญใหม่อีกครั้ง" ลงมาเป็น scope ตอน QA ผ่าน `F-002/ux-wireframe.md §7` (finding #32) โดย**ไม่เคยผ่าน AC**
> · qa จับได้และส่งมาให้ product เคาะ (**N-3** ใน `F-002/gate-e-verdict.md §6`) · คำตัดสิน + เหตุผลเต็ม:
> [`F-002/release-gate-f.md §7.2`](F-002/release-gate-f.md)
>
> - **จำนวน AC ของ F-002: 35 → 36** · การ remap ตาราง AC↔เคสเป็นสิทธิ์ของ **@qa** ไม่ใช่ของ product
> - **หลักฐานวันนี้ของข้อนี้ = unit 8 เคส** (`MembersScreen.reinvite.test.tsx`, รันจริงบน CI) ·
>   **ยังไม่มีเคสเบราว์เซอร์** ⇒ อ่อนกว่า AC เพื่อนบ้าน · ไม่บล็อก dogfood (server บังคับสิทธิ์ทุกทางอยู่แล้ว) ·
>   เพิ่มเคสเบราว์เซอร์ = งานค้างของ qa, trigger = **ครั้งถัดไปที่แตะจอสมาชิก (F-003/F-004)**
> - **กติกาต่อจากนี้:** ของที่ผู้ใช้เห็นและกดได้ซึ่งลงมาหลัง Gate 2 ⇒ **ต้องกลับมาขอ AC จาก product ก่อน implement**
