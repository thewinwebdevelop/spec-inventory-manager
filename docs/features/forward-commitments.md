...
## → จาก F-003 Gate 1 re-baseline (user sign-off 2026-09-27 · D-032/D-033)

> รายละเอียดเต็มอยู่ [F-003/F-003.md §7](F-003/F-003.md) — แถวที่นี่คือของที่ **ออกนอก F-003** เท่านั้น

| งานเปิดค้าง | เจ้าของ | **TRIGGER** | binding |
| --- | --- | --- | --- |
| **D-033 — ช่องต้นทุน/กำไร/COGS/มาร์จิ้น ทุกจอ gate ด้วย `view_financials`** + test ว่า Staff preset ไม่เห็น (API + web + mobile) | product + backend-api | **Gate 1 ของ feature แรกที่มีช่องต้นทุน** (คาด F-010) แล้วทุก feature ถัดไป (F-011, F-030 …) | D-033 |
