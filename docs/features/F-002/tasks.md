...
### CI ค้าง 2 รอบติดที่ขั้นเดียวกัน — และ workflow **ไม่มี `timeout-minutes` เลยสักที่** (2026-08-19)

`e2e-web` ค้างที่ **install Playwright browsers** สองรอบติด: **68 นาที** (run 32271380129) และ **26 นาที** (run 32278047892)
· ขั้นเดียวกันนี้ใช้เวลา**ไม่ถึงนาที**ในทุกรอบก่อนหน้า · ในรีโปไม่มีอะไรเปลี่ยน ⇒ **การดาวน์โหลดเบราว์เซอร์ค้าง ไม่ใช่เทสต์แดง**

**ปัญหาที่ใหญ่กว่าตัวค้างเอง:** ทั้งไฟล์ `ci.yml` **ไม่มี `timeout-minutes` แม้แต่ job เดียว** ⇒ ขั้นที่ค้างจะวิ่งไปจนชน default **6 ชั่วโมง** ของ GitHub
⇒ เลนนั้น**ไม่รายงานอะไรเลย** ไม่แดง ไม่เขียว **หายไปเฉย ๆ** · สัญญาณเดียวที่มีคือคนบังเอิญเห็น spinner หลังผ่านไปชั่วโมงหนึ่ง

> เป็นรูปเดียวกับ "เขียวเพราะไม่เคยรัน" ที่ I-37 มีไว้จับ แต่คนละหน้า: คราวนี้คือ **"ไม่มีผลเพราะไม่เคยจบ"**

**แก้ 2 ชั้น:**
1. **ทุก job มีขอบ** — 9 job ตั้งจากเวลาที่วัดได้จริงคูณเผื่อ (oasdiff 8 วิ → 15 นาที · mobile-e2e 8.5 นาที → 45) · ขั้นที่ค้าง**กลายเป็นผลลัพธ์**
2. **ขั้น install ลองซ้ำแบบมีขอบ** — 3 ครั้ง ครั้งละ ≤220 วิ · การค้างแบบนี้เป็นของชั่วคราว การ retry ที่มีขอบเปลี่ยนมันเป็นเขียวแทนที่จะให้คนมากด rerun เอง

**ตัวเลข timeout เป็นนโยบายของ @devops** — ผมตั้งจากที่วัดได้และเผื่อไว้กว้าง ปรับได้ตามที่เห็นควร
...
### guard ตัวนี้จับ **ผมเอง** ในรอบถัดมา — และคำตอบคือแก้กฎ ไม่ใช่แก้บันทึก

CI แดงที่ `node-ci` ด้วยสองบรรทัดนี้:
```
apps/mobile/lib/auth/screenshot_guard.dart  ← docs/features/F-002/tasks.md
packages/db/tenancy.ts                      ← docs/features/F-002/tasks.md
```
⇒ **คือย่อหน้าที่ผมเพิ่งเขียนอธิบายการแก้นั่นเอง** — มัน quote path เก่าไว้ในเครื่องหมาย backtick **เพื่อจะบอกว่ามันผิด**

**ทำไมเครื่องผมเขียว:** ผมรัน suite **ก่อน** เขียนย่อหน้านั้น · เขียวในเครื่อง แดงที่อื่น ด้วยเหตุผลเดิมที่บันทึกไว้ตั้งแต่ F-001

**กฎที่ถูกไม่ใช่ "ยกเว้น path นี้" แต่คือ "ยกเว้นตามคนอ้าง":**

| ผู้อ้าง | อ้างไฟล์ที่ย้ายไปแล้ว |
|---|---|
| `.md` (บันทึก/เอกสาร) | **ได้** — log ที่เล่าว่าไฟล์เคยอยู่ไหน ไม่ใช่ความผิด · **แก้ log = ปลอมบันทึก** |
| `.kt` `.swift` `.ts` `.dart` (โค้ด) | **ไม่ได้เด็ดขาด** — คอมเมนต์คือการนำทาง มีคนเดินตาม และนั่นคือเหตุผลทั้งหมดที่ guard นี้เกิด |
...
## design system drift — user เห็นเองว่า "padding เพี้ยนหมด" (2026-09-01)

ยกสแตกให้ user เล่นจริง แล้ว user บอกว่า padding เพี้ยนและ design system ดูไม่ตรง — **ถูกทั้งสองข้อ และหาสาเหตุเชิงระบบเจอ**

### สาเหตุ: เอกสาร design system เดินหน้าไปแล้ว ชั้น token ของเว็บไม่ได้เดินตาม

`tokens.css` เขียนไว้ที่หัวไฟล์เองว่า *"ถ้า token ขาด = gap ที่ต้องคุยกับ ux ไม่ใช่ค่าที่จะคิดเอง"* — **ไม่มีอะไรตรวจประโยคนั้น**
D-031 / T-002-X1 (ux ทำเสร็จ ✅) เติม token เข้าเอกสาร แล้ว **12 ตัวไม่เคยลง CSS เลย**:
`size.dialog.max-w` · `size.sidebar.w` · `size.list-row.min-h` · `size.icon.sm/md/lg/xl` · `type.button.sm` · `color.info.*` · `color.badge.neutral.*` · `focus.ring.*`

⇒ คนเขียนจอเลย**พิมพ์ตัวเลขเอง** ซึ่งเป็นสิ่งที่ไฟล์ห้ามไว้ตรง ๆ

### 🔴 บั๊กที่มองเห็นด้วยตา: ทุก dialog แคบกว่าที่ออกแบบ 80px

`ConfirmDialog` ใช้ `max-w-[400px]` = **ความกว้างของ auth card** บน dialog ที่เอกสารบอกว่า 480px
· และเอกสารเขียนเตือนไว้เองว่าสองตัวนี้ **"คนละตัวกับ"** กัน ⇒ ยืนยันถอดสมาชิก/เปลี่ยนสิทธิ์/ออกจากร้าน แคบหมด

### แก้ 3 ชั้น

1. **เติม token ที่ขาด 12 ตัว** ลง `tokens.css` (คัดลอกค่าจากเอกสาร ไม่ได้คิดค่าใหม่ — ค่าเป็นของ ux)
2. **แทน literal ด้วย token 8 จุด** — `max-w-[480px]`×6 · `min-h-[56px]` · และแก้ dialog 400→480
3. **guard `tokens.contract.test.ts`** — parse ตาราง token ใน `design-system.md` แล้วบังคับว่าทุกตัวต้องมีใน `tokens.css`
   · mapping ไม่ได้คิดเอง = ตาราง §1.5 ของเอกสาร · namespace ที่ไม่ map ต้อง**เขียนเหตุผล** (`icon.*`=ชื่อ Phosphor · `bp.*`/`space.N`=ของ Tailwind เอง)
   · **mutation สองทาง**: ลบ token ที่เอกสารประกาศ ⇒ แดง · เพิ่ม namespace ใหม่ที่ไม่มีใคร map ⇒ แดง

web **337** เขียว · typecheck/lint สะอาด

### ⚠️ ที่ยังไม่ได้แก้ และใหญ่กว่าเรื่อง token: **คอมโพเนนต์ถูกสร้างตาม wireframe ไม่ใช่ตาม mockup**

เทียบจอ S4 (ข้อมูลร้าน) ของจริงกับ mockup ที่ sign-off ไว้ (D-026):

| | mockup | ของจริง |
|---|---|---|
| แถวข้อมูล | label ซ้าย · ค่าขวา · มีเส้นคั่น | ซ้อนกันบนล่าง ไม่มีเส้นคั่น |
| "แก้ไข" | **ปุ่มจริง** ชิดขวา | ลิงก์ขีดเส้นใต้ |
| sidebar | รายการสูง 44px · ตัวที่ active มีพื้น `surface.muted` | ข้อความเปล่า ไม่มีพื้น ชิดกัน |
| ตัวสลับร้าน | คอนโทรลมีขอบ + chevron | บล็อกเปล่า |
| การ์ด | padding ในการ์ดเต็มตามสเปก | ตัวอักษรชิดขอบการ์ด |

⇒ **การเติม token แก้พื้นฐานให้ถูก แต่ไม่ได้ทำให้จอกลายเป็น mockup** — ส่วนนี้เป็นงาน**ประกอบคอมโพเนนต์ใหม่ตาม mockup** ซึ่งเจ้าของคือ **ux (spec) + frontend (ทำ)** และเป็นชิ้นใหญ่กว่ารอบนี้มาก ⇒ **ต้องให้ user เคาะก่อนว่าจะลุยต่อไหม**

### ประกอบ 5 จอใหม่ตาม mockup (user สั่ง "ทำทั้งหมดให้ครบ" 2026-09-01)

ถอด pattern จาก `docs/design-system/mockup/f002-org.html` แล้วสร้าง **primitive 7 ตัว** ที่ mockup มีแต่โค้ดไม่มี:
`SectionCard` (`.card`) · `DataRow` (`.drow`) · `ListRow`+`Avatar` (`.lrow`) · `Badge` · `Banner` · `NavItem` (`.navi`) · `IconButton`
· `Button` เพิ่ม `size="sm"` และ `variant="tertiary"` ที่ D-031 กำหนดไว้แต่ไม่เคยมี

**จอที่ประกอบใหม่:** S3 AppShell+switcher · S4 ข้อมูลร้าน · S6 สมาชิก · S8 แผ่นลิงก์ · S11 /invite · + panel อีก 7 ตัวที่พิมพ์ `rounded-card border border-border-default p-4` เหมือนกันหมด

### 🔴 บั๊กที่เจอระหว่างทาง: `cn()` กลืนคลาสสีของ tertiary ทิ้ง

`twMerge("text-primary","text-button-sm")` ⇒ `"text-button-sm"` — tailwind-merge ไม่รู้จัก `--text-*` ของโปรเจกต์ เลยเดาว่า `text-อะไรก็ตาม` คือ**สี** แล้วเก็บตัวหลังไว้ตัวเดียว
⇒ `tertiary` ซึ่งนิยามว่า *"ตัวเล็ก + สี primary"* **เสียสีตั้งแต่บรรทัดที่เขียน** ปุ่มเงียบที่สุดของระบบเลยออกมาเป็นตัวหนังสือธรรมดา

**ผมเกือบรายงานผิด** — ตอนแรกเขียนว่า "กลืนทั้งโปรเจกต์" แล้วไปวัด computed style จริงบนหน้า ⇒ **type token ทุกตัวเรนเดอร์ถูก** (14/22 · 18/28 · 24/32)
· เพราะ `className` แบบ static ไม่ผ่าน twMerge เลย — Tailwind ปล่อยทั้งสองคลาสและมันตั้งคนละ property
· ที่โดนคือเฉพาะสตริงที่วิ่งผ่าน `cn()`/`cva` เท่านั้น ⇒ **ขอบเขตแคบกว่าที่ผมพูดไปมาก**

**แก้ที่ราก:** `extendTailwindMerge` ลงทะเบียนชื่อ type token ทั้ง 7 ตัวเป็น `font-size` ⇒ สีกับขนาดกลับมาเป็นคนละคำถาม
· เทสต์ `utils.test.ts` 4 เคส: สี+ขนาดอยู่รอดทั้งสองลำดับ · **สองขนาดยังชนกัน** · **สองสียังชนกัน** (ไม่ได้ปิด merge ทิ้ง) · และเคสที่ไล่ token ทุกตัว

web **341** เขียว · typecheck/lint สะอาด

### 🟠 ค่าที่ยังไม่มีใน design-system — **ส่งต่อ @ux ไม่คิดเอง**

design-system §6 เขียนว่า token ที่ขาด = gap ที่ต้องคุยกับ ux ไม่ใช่ค่าที่จะคิดเอง ⇒ ผมทิ้งไว้เป็น literal และรวมไว้ที่เดียว:

| ค่า | ที่ | คืออะไร |
|---|---|---|
| `150px` / `180px` | `DataRow` | ความกว้างคอลัมน์ label / ความกว้างต่ำสุดของค่า (มาจาก mockup) |
| `.62` | `ListRow` | ความจางของแถวที่ถูกถอด (`.dim` ของ mockup) |
| `640px` | จอความปลอดภัย | ความกว้างหน้าเนื้อหา |
| `360px` · `z-100` | `Toast` | ความกว้าง + ชั้นซ้อน |
| `90%` | `ConfirmDialog` | ความกว้างบนจอแคบ |

### user เจอเองอีก 2 ข้อ: switcher ไม่ใช่ popover · ยุบ sidebar ไม่ได้ (2026-09-01)

**1 · dropdown ของตัวสลับร้านอยู่ในสายการไหลปกติ** ⇒ เปิดแล้ว**ดันเมนูทั้งแถบลงไป** แทนที่จะลอยทับ
mockup เขียนไว้ตรงตัวที่ `.dd`:
```css
position:absolute; top:calc(100% + 6px); width:290px;
box-shadow:var(--shadow-dialog); z-index:40
```
⇒ ทำเป็น popover ตามนั้น · เพิ่ม `.ck` ช่องเครื่องหมายถูก 20px (คงที่ทุกแถว ชื่อร้านจะได้ตรงกัน) · เส้นคั่นก่อน "สร้างร้านใหม่" เพราะเป็นการกระทำคนละชนิด · แถวสูง `size.list-row.min-h`

**2 · ยุบ/กาง sidebar ไม่ได้เลย — และ §8.2 สั่งไว้ว่าต้องได้**

design-system §8.2 เขียนว่า AppShell nav ที่ **md (768–1023)** ต้อง *"ยุบเป็น top bar + drawer (hamburger)"*
⇒ **ไม่เคยมีของนี้เลย** — ต่ำกว่า `md` เมนูแค่ซ้อนบนหน้าเป็นบล็อกเต็มความกว้าง ไม่มีทางเก็บ และไม่มีปุ่มยุบที่ความกว้างไหนเลย
⇒ ทำ top bar + ปุ่ม hamburger · เปิดค้างที่ ≥lg ตาม *"sidebar ถาวรซ้าย"* ของ §8.2 · กดลิงก์ในลิ้นชักแล้วปิดเอง (ไม่งั้นลิ้นชักบังหน้าที่เพิ่งขอ)
· **breakpoint เปลี่ยนจาก `md` เป็น `lg`** ให้ตรงกับ §8.2 (ของเดิมสลับที่ 768px ซึ่งเป็นขอบล่างของช่วง tablet ไม่ใช่ขอบบน)
...
### 🔴 user จับได้ว่า **"dialog" 7 ตัวไม่ใช่ dialog เลย** (2026-09-01)

user ส่งภาพหน้าจอเต็มมาให้ดู: กด "กรอกข้อมูลผู้เสียภาษี" แล้ว**ฟอร์มไปโผล่ล่างสุดของหน้า** ใต้การ์ดอื่นและใต้ปุ่ม "ออกจากร้านนี้" — ไม่ใช่ลอยทับกลางจอ

ตรวจแล้ว: **7 จาก 8 ตัวไม่มี overlay** — มีแต่ `ConfirmDialog` ตัวเดียวที่ทำถูก
`InviteDialog` · `TaxProfileDialog` · `ChangeRoleDialog` · `RenameOrgDialog` · `RemoveMemberDialog` · `LeaveOrgDialog` · `CopyLinkPanel` ทั้งหมดคืน `<div role="dialog">` ธรรมดาลงในสายการไหลของเอกสาร
⇒ ui.md §1 ระบุ S5/S7/S8/S9 ว่าเป็น **dialog** บนเว็บ ⇒ **ไม่ตรงสเปกมาตั้งแต่ต้น** และ `role="dialog"` ที่ติดไว้ก็ยิ่งทำให้ screen reader เข้าใจผิด

**ทำไมผมไม่เห็น:** ผมวัด computed style ของ element ทีละตัว (`padding: 24px` ✓ `min-height: 44px` ✓) แล้วดูภาพที่ crop ไว้ — **ไม่เคยดูทั้งหน้า**
· ที่แย่กว่านั้นคือ **ภาพจอเชิญสมาชิกที่ผมถ่ายเองเมื่อรอบก่อนก็เห็นฟอร์มอยู่ล่างสุดแล้ว** และผมอ่านมันเป็น "การ์ดสวยขึ้น" แทนที่จะเป็น "นี่ไม่ใช่ dialog"
⇒ **การวัดค่าที่ถูกทีละตัว ไม่ได้แปลว่าหน้าถูก** — เป็นรูปเดียวกับ "แต่ละฝั่งสอดคล้องกับตัวเอง" ที่เจอมาทั้ง feature นี้ แค่ย้ายมาอยู่ที่ระดับสายตา

**แก้:** `DialogShell` ตัวเดียว — scrim `color.overlay` · กลางจอ · `size.dialog.max-w` · Escape ปิด · คลิกฉากหลังปิด (แต่ลากเลือกข้อความจากในกล่องออกมาไม่ปิด) · โฟกัสเข้ากล่องตอนเปิดและ**คืนโฟกัสให้ปุ่มที่เปิดมันตอนปิด**
· โฟกัสที่ตัวกล่อง ไม่ใช่ช่องแรก — ฟอร์มที่เด้งเคอร์เซอร์เข้าช่องก่อนคนอ่านหัวข้อเสร็จคือการแย่งอ่าน
· ทั้ง 7 ตัวย้ายมาใช้ตัวเดียวกัน

web **341** เขียว · ยืนยันด้วยภาพ**ทั้งหน้า** ไม่ใช่ crop

### user ชี้ต่อ: การ์ดภาษี padding/typography ไม่ตรง — **ผมแปลงแค่เปลือก ไม่ได้แปลงเนื้อใน** (2026-09-01)

รอบก่อนผมเปลี่ยน `TaxProfileCard` แค่ `className` ข้างนอกให้เป็น card treatment แล้วถือว่าเสร็จ
· ข้างในยังเป็น **`<dl>/<dt>/<dd>` เปล่า ๆ** ⇒ ป้ายกับค่าสลับกันเต็มความกว้าง ไม่มีคอลัมน์ ไม่มีเส้นคั่น ⇒ **ข้อมูล 4 อย่างอ่านเป็น 8 บรรทัดที่ไม่เกี่ยวกัน**
· ปุ่ม "แก้ไข" เป็นบล็อกอยู่**ใต้หัวข้อ ชิดซ้าย** ขณะที่การ์ดร้านข้างบนมันอยู่**มุมขวาบน** ⇒ การ์ดสองใบติดกันคนละภาษา
· ป้ายใช้ `type.body.sm` แทน `type.label.sm` + `text.muted`

⇒ แปลงเป็น `SectionCard` (หัวข้อ + action ขวาบน) + `DataRow` ทั้งใบ · ปุ่มเปิดเลขเป็น `size="sm"`

**กวาดหาที่เหลือแบบเดียวกันด้วย ไม่รอให้ชี้:**
- `<dl>` ที่เหลือทั้งรีโป: **ไม่มีแล้ว**
- **S1 เลือกร้าน** — แต่ละร้านเป็นกล่องมีขอบของตัวเอง และใช้คลาส **`border-border` ซึ่งไม่มีอยู่จริง** (token คือ `border-border-default`) ⇒ ที่เห็นเป็น "ขอบ" คือค่า default ของ Tailwind ไม่ใช่ token ⇒ ยกเป็น `SectionCard` + `ListRow` + `Avatar`
- สแกน `border-border\b` ทั้ง tree: เหลือแต่ `border-border-default` ที่ถูกต้อง ✅

> บทเรียนซ้ำของรอบนี้: **แก้เปลือกแล้วดูรูป crop = เห็นว่ามันดีขึ้น แต่ไม่เห็นว่ามันยังไม่ตรง** — เหมือนตอน dialog ที่ไม่มี overlay เป๊ะ

web **341** เขียว

### 🟠 B-14 filed: `/` ยังเป็น placeholder ของ F-000 — และกฎที่ web.md เขียนไว้ **อ้างของที่ไม่มีอยู่จริง** (2026-09-01)

user ถามว่า "ล็อกอินไว้แล้วแต่ยังเข้าหน้า login ได้ คือยังไม่ถึงเฟส auth guard หรือบั๊ก" ⇒ ไล่ดูแล้ว **เป็นคนละเรื่องกันสองข้อ**

**ข้อที่ถาม — ไม่ใช่บั๊ก:** ไม่มีสเปกไหนสั่งว่าคนที่มี session อยู่แล้วเข้า `/login` ต้องถูกเด้งออก
· ทิศที่มีแล้วคือ `OrgGuard` (ไม่ล็อกอิน → `/login`) ✅ · `RouteGuard` ที่ CLAUDE.md พูดถึงคือ **capability gating** ผูกไว้ที่ F-003/F-007 คนละเรื่อง

**ข้อที่เจอระหว่างตรวจ — เป็นบั๊กตามสเปก:**
`docs/architecture/web.md:86` → `"/" → redirect ตาม SessionState (none→/login · authed→/o/[defaultOrg])`
ของจริง `apps/web/src/app/page.tsx` ยังเป็น T-000-09 placeholder ⇒ **เปิดโดเมนเปล่าเจอหน้า placeholder ไม่ว่าจะล็อกอินอยู่หรือไม่**

⇒ **รูปเดียวกับ B-6 เป๊ะ** (redirect หลัง login ชี้ placeholder ของ F-000) · B-6 ถูกจับได้เพราะ E-01 เดิน login→shop เป็นเส้นเดียว · **ไม่มีเทสต์ไหนเดินผ่าน `/` เลย** มันเลยรอด

**ทำไมแก้เองไม่ได้ — `defaultOrg` ไม่มีอยู่ที่ไหนในรีโปเลยนอกจากบรรทัดนั้น**
· ที่ค้นเจอทั้งหมดคือ `DEFAULT_ORG_PLAN_KEY` = แผนที่ร้าน**ใหม่**ถูก provision — คนละเรื่องสิ้นเชิง
· โมเดล F-002 **ไม่มี "ร้านเริ่มต้น"**: คนอยู่ 0/1/หลายร้าน ซึ่งคือเหตุผลที่ login เด้งไป `/select-org` แล้วให้จอเลือกร้านตัดสิน
⇒ **web.md อธิบายกฎ routing ที่สร้างบนฟิลด์ซึ่ง data model ไม่เคยมี** — รูปเดียวกับ audit "ข้อเท็จจริงที่ไม่มีใครรองรับ" ทั้งชุด

**@product ต้องเคาะ:** `/` ควรทำเหมือน login คือไป `/select-org` · หรือจะทำให้ "ร้านเริ่มต้น" มีจริง (= ฟิลด์ใหม่ + ผิว contract ใหม่ + กฎว่าจะเกิดอะไรเมื่อร้านนั้นถูกถอดสิทธิ์)
· ผม**ไม่เขียนเทสต์ให้** เพราะการเขียนเทสต์ = การเลือกปลายทาง ซึ่งเป็นสิ่งที่ยังเปิดอยู่พอดี

### ✅ B-14 ปิด: product เคาะแล้ว — `/` ทำเหมือน login (2026-09-04)

**คำตัดสินของ user:** ยังไม่ล็อกอิน → `/login` · ล็อกอินแล้ว → `/select-org`
⇒ เลือกทางที่ **ไม่**สร้าง "ร้านเริ่มต้น" ขึ้นมาใหม่ ⇒ ไม่มีฟิลด์ใหม่ ไม่มีผิว contract ใหม่ ไม่มีกฎ revoke ที่ต้องคิด
⇒ `docs/architecture/web.md:86` แก้ให้ตรงกับของจริงแล้ว (`authed→/select-org · unknown→รอ`) — บรรทัดนั้นคือที่เดียวในรีโปที่เคยพูดถึง `defaultOrg`
...
`omni_rt` เป็น `Path=/auth` (D-019) ⇒ โหลดเย็น ๆ ครั้งแรก **ระบบยังไม่รู้จริง ๆ** ว่าล็อกอินอยู่ไหม จนกว่า bootstrap refresh จะจบ
· ถ้าอ่าน `unknown` เป็น "ยังไม่ล็อกอิน" ⇒ **คนที่ล็อกอินอยู่จะถูกเด้งไป `/login` ทุกครั้งที่รีโหลด `/`** — บั๊กที่ดูเหมือนฟีเจอร์ทำงานถูก
· `decideRoot` จึงมี `wait` เป็นสถานะจริง ไม่ใช่ default ที่ตกไปทาง sign-in

**ทำไมต้องมี E-15 ทั้งที่ pure fn มีเทสต์แล้ว:** เพราะสิ่งที่พลาดจริง ๆ ไม่ใช่ตรรกะ — **ไม่มีใครเคยเปิด `/` เลย** ต่างหาก
เทสต์ของ `decideRoot` จะเขียวได้สบายแม้ `page.tsx` ยังเป็น placeholder อยู่ · E-15b จึงยืนยันด้วยว่า string `"apps/web placeholder shell"` **หายไปแล้ว** ไม่ใช่แค่ถูก redirect ทับ

web **344** เขียว · lint/typecheck สะอาด · `build-defects` 6 เขียว (B-14 ย้ายจากแถวเปิด → แถวปิดพร้อม pin)

### 🔬 manual pass §12.2 เดินรอบแรก — เจอของใหม่ 4 ข้อ (2026-09-04)

**ผลเต็มพร้อมหลักฐานอยู่ที่ [manual-pass-results.md](manual-pass-results.md)** · runbook ติ๊กครบแล้ว

| | ผล |
|---|---|
| M-03 · M-05 · M-06 | ✅ ผ่าน |
| M-04 | ⚠️ ครึ่งเดียว — ธงถ้อยคำผ่าน · "ใครรับเมื่อไหร่" ไม่มี (**B-15**) |
| M-02 | ⚠️ ตอบคำถามที่ runbook ถามไม่ได้ — **ไม่มีจอไหนเรียก endpoint นี้เลย** |
| M-01 | 🔴 ต้องใช้คน (LINE + เครื่องจริง) — ตัดข้อสงสัยเรื่องรูปร่างลิงก์ให้แล้ว |
| M-07 · M-07ข · M-07ค | 🔴 **BLOCKED — B-18** |

**สิ่งที่รอบนี้สอน:** ทั้ง 4 ข้อที่เจอ มี suite เขียวทับอยู่ครบทุกชั้น — web 363 · api 684 · mobile 440 ·
browser lane · emulator lane · และไม่มีเลนไหนแดงสักเลนเดียว
· **B-16 คือรูปที่คมที่สุด: ปุ่มที่ไม่มีอยู่ ไม่ render อะไร ไม่ assert อะไร ไม่ throw อะไร** — เครื่องมองไม่เห็น "ของที่หายไป"
คนกดหาทางออกจากระบบสิบวินาทีก็เจอ

#### B-15 — แถวคำเชิญที่ตายแล้วยังบอกว่าลิงก์ยังใช้ได้ ✅ แก้แล้ว

ยกเลิกคำเชิญ → dialog บอก *"ลิงก์ที่ส่งไปแล้วจะใช้ไม่ได้ทันที"* → แถวที่เหลือไว้เขียนว่า
**"ลิงก์ใช้ได้ถึง 11 ก.ย. 2569 20:36 (อีกประมาณ 7 วัน)"** · แถวที่ถูกรับไปแล้วก็พูดแบบเดียวกัน

`expiry.ts` **เตือนเรื่องนี้ไว้ใน doc comment ของตัวเอง** (*"telling somebody a dead link is live"*)
แล้ว `MembersScreen` เรียก `formatExpiry` กับทุกแถวโดยไม่ดู `status` — `expiresAt` เป็น NON-NULL ทุกแถวตามสัญญา (ux Q14)
มันจึงอ่านได้เสมอและผิดเสมอ ⇒ **โมดูลรู้กฎ ≠ จอทำตามกฎ**

แก้ตาม **ux-wireframe §7 บรรทัด 549 ที่สั่งไว้อยู่แล้ว** (ไม่ใช่ copy ใหม่) + §14 บรรทัด 1091 (เลย `expiresAt` = "หมดอายุแล้ว" ทันที)
· `acceptedAt` ถูกอ่านขึ้นจอครั้งแรก ⇒ ปิดครึ่งที่ขาดของ M-04
· เทสต์ 12 + 3 (มีเคส non-vacuity: แถวเป็น ๆ ต้องยังมีบรรทัดหมดอายุ) · red→green ยืนยันแล้ว

#### B-16 — ทั้งแอปเว็บไม่มีปุ่ม "ออกจากระบบ" ✅ แก้แล้ว

`orgTh.shell.nav.logout = "ออกจากระบบ"` อยู่ในดิกชันนารี **และไม่มีใครเรียกเลยทั้ง tree**
· ux-wireframe §S2 วาดไว้เป็นแถวสุดท้ายของ sidebar · ทางเดียวที่ออกได้จริงคือ "ออกจากระบบทุกอุปกรณ์"

แก้ด้วย `NavAction` + `logoutDevice()` · **ล้มแล้วอยู่กับที่และบอก** (ไม่เด้งไป `/login` แบบหลอก เพราะ token
ถูกเคลียร์หลัง `204` เท่านั้น ⇒ ถ้าล้ม session ยังอยู่ครบ) · **E-16** บนเบราว์เซอร์จริงพิสูจน์ว่า *session ตายจริง*
ด้วยการโหลด `/` ใหม่ (refresh cookie เป็น httpOnly — component test มองไม่เห็น) · พื้นเลน browser **28 → 29**

#### B-17 — แอปมือถือชี้ไป API ไหนไม่ได้เลย ✅ แก้แล้ว

`lib/main.dart` ฮาร์ดโค้ด `http://localhost:3000` · `--dart-define=API_BASE_URL` ที่ **ทั้งเลน CI และ runbook ส่งเข้าไป**
ถูกอ่านโดย `integration_test/org_flow_test.dart` ไฟล์เดียว ⇒ ใน emulator/มือถือจริง `localhost` = ตัวเครื่อง ⇒ ทุก request ตาย

จอ login รายงานว่า "เกิดข้อผิดพลาด กรุณาลองใหม่อีกครั้ง" เพราะ `catch (_)` กลืน exception ทั้งหมด
(สตริง `errorNetwork` มีอยู่ใน ARB แต่เส้นนี้ไม่เคยใช้) ⇒ **ผมไล่ผิดทางอยู่หลายรอบ** สงสัย cleartext HTTP ก่อน
(ลองเปิด `usesCleartextTraffic` แล้ว — ไม่ใช่) กว่าจะไปเจอว่า define ไม่เคยถูกต่อ

**ทำไม CI เขียวมาตลอด:** E-10 สร้าง provider graph เองจาก define ไม่เคยผ่าน `main.dart`
⇒ harness กับแอปต่างคนต่างสอดคล้องกับตัวเอง แล้วไม่ตรงกัน — รูปเดียวกับ audit ทั้งชุด

tripwire: `test/app/base_url_define_test.dart` บังคับให้ **แอป · integration test · runbook** พูดถึงคีย์เดียวกัน
(เทสต์พฤติกรรมจับไม่ได้ — `String.fromEnvironment` เป็น compile-time const) · red→green ยืนยันแล้ว

#### 🟠 B-18 filed — จอ F-002 ทั้งชุดบนมือถือไม่มีทางเข้าถึงในแอปจริง (**เปิด · product**)

หลังแก้ B-17 ล็อกอินสำเร็จจริงบน emulator แล้ว **แอปพาไป `SecurityScreen` ของ F-001 แล้วจบ**

```
CreateOrgScreen · MembersScreen · OrgPickerScreen · OrgProfileScreen
  → 0 อ้างอิงจากที่อื่นใน apps/mobile/lib/ ทั้งสี่ตัว
```

**นี่คือ B-4 อีกชั้นหนึ่ง** (B-4 = provider ไม่ถูก wire · ปิดไปแล้ว ⇒ ชั้นนี้ = provider ถูก wire แต่ไม่มี*จอ*ไหนถูก wire)
และเป็นรูปเดียวกับ **B-14** บนเว็บ · E-10 สร้าง `MembersScreen` ขึ้นมาเองใน `MaterialApp` ของตัวเอง
⇒ พิสูจน์ว่า**จอทำงานเมื่อมีคนพามันขึ้นมา** ไม่ได้พิสูจน์ว่า**มีใครไปถึงมันได้**

**ผมไม่แก้เอง** — `app.dart` ประกาศเองว่า navigation จริงเป็นของ **F-006** และเป็นคำถาม scope แบบเดียวกับ B-14
⇒ **@product เคาะ:** (ก) F-002 ต่อ nav ชั่วคราวให้ถึงจอพวกนี้ หรือ (ข) ยอมรับว่ามือถือของ F-002 ยังใช้ไม่ได้จนถึง F-006
แล้วแก้ CHANGELOG/AC ให้ตรง · **ถ้าเลือก (ข) M-07/ข/ค ต้องเลื่อนไป F-006 ทั้งชุด** — ไม่ใช่ "ยังไม่ได้ทำ" แต่คือ "ทำไม่ได้"

#### สิ่งที่ปิดได้ระหว่างทาง: **OS ทำตาม `FLAG_SECURE` จริง**

runbook เขียนเองว่า *"พิสูจน์ไม่ได้ว่า OS ทำตาม"* — ปิดแล้วบน Android 13 จริง:
จอ login ของเรา `screencap` → **exit 1 · 0 ไบต์** · จอ Home ของ Android → **exit 0 · 1.36 MB** ·
...
### 🧭 ปิดข้อค้างที่รอ user เคาะ — user มอบให้ตัดสินแทน (2026-09-05)

| # | เรื่อง | ตัดสิน |
|---|---|---|
| 1 | `apps/api/CLAUDE.md` บรรทัด 8 บอกว่า `APP_ROLE` มีอยู่แล้ว | **แก้** — `grep` ทั้ง `src/` + `packages/*/src` = **0 ครั้ง** ⇒ เขียนใหม่ว่าเป็น target ที่เกิดตอน F-021/F-023 · *(แตะ protected path ครั้งเดียวภายใต้การมอบหมายนี้ และเป็นการแก้ข้อเท็จจริงล้วน)* |
| 2 | ธง "บัญชีสร้างหลังออกลิงก์" ย้ายไปแถวสมาชิก | **ไม่ทำตอนนี้** — ต้องเพิ่ม field ใน `MemberRow` = ผิว contract ใหม่ + สองไคลเอนต์ + ข้อเท็จจริงเดียวอยู่สองที่ · อาการจริงที่ manual pass เจอคือ**หาไม่เจอ** ซึ่ง ux ปิดไปแล้วด้วยการเปลี่ยนป้ายปุ่ม ⇒ forward-commitment, trigger = **F-004 Gate 2** |
| 3 | M-02: `/login/help` ชี้ไปทางที่ไม่มีอยู่ | **แก้ copy ทั้งสองแอป** — ux-wireframe §12.3 ตัดสินไว้แล้วว่าทางไปต่อคือ "ติดต่อทีมงาน OmniStock" และ §13 มีกฎของตัวเองว่า **UI ห้ามสัญญาสิ่งที่ยังไม่มี** ⇒ ของเดิมผิดกฎของ ux เอง · กลับมาแก้อีกครั้งเมื่อ F-004 มีจอจริง |
| 4 | `@phosphor-icons/react` | **ไม่ติดตั้ง** — ยังไม่เคยถูกติดตั้งอยู่แล้ว · `components/ui/Icon.tsx` ครอบทุกไอคอนที่ F-002 ใช้ · dependency ใหม่ต้องผ่าน PM (quality-gate §E) และ "สวยขึ้นนิดหน่อย" ไม่คุ้มผิวที่เพิ่ม |
| 5 | ปุ่มสลับ dark theme | **ไม่ทำใน F-002** — ทั้งสองแอปตามระบบปฏิบัติการอยู่แล้ว (ถูกตาม design-system §1.1b) · ปุ่มสลับต้องตัดสินว่า**เก็บที่ไหน** (ต่อเครื่อง/ต่อบัญชี/sync) ซึ่งเป็นคำถามของหน้าตั้งค่า = F-006 |
| 6 | เวอร์ชัน npm `0.0.0` vs pubspec `1.0.0+1` | **`pubspec` → `0.0.0+1`** — `1.0.0` คือ default ของ `flutter create` ที่ไม่มีใครแก้ ⇒ อ้างเวอร์ชันที่ไม่เคย ship · `+1` คงไว้เพราะสโตร์บังคับ · **นี่ไม่ใช่การตัดเวอร์ชัน แต่คือการหยุดอ้างเวอร์ชัน** — `release` ยังเป็นคนตัดเลขจริงตอนปิด Phase 1 |

ทั้งหกข้อเขียนเหตุผล + trigger ไว้ที่ `docs/features/forward-commitments.md` และ `release-gate-f.md` §1/§5

**และเก็บของที่ ux ทิ้งไว้ให้ frontend ทันทีในรอบเดียวกัน** — เพราะ ux เตือนเองว่า
*"คีย์ที่ยังไม่มีใครเรียก คือรูปเดียวกับ B-16 เป๊ะ"* และ B-16 คือบั๊กที่เพิ่งปิดไปในรอบนี้:

- `shell.nav.menuLabel` → `AppShell` 2 จุด (เดิม hardcode สตริงไทยไว้ในคอมโพเนนต์)
- `leaveLastOwnerCta` / `leaveLastOwnerHere` → `LeaveOrgDialog` แตกสองสาขาตาม §10.3
  · **origin เป็น prop ไม่ใช่ `usePathname()`** — คนเรียกรู้ว่าตัวเองอยู่จอไหน ส่วนการเทียบสตริง route
