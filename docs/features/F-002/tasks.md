...
| T-002-12 | ★ ขยาย `SecurityEventType` union **15 ค่า** (รวม `org.member.left`, `org.tax_profile.revealed`, `auth.password.admin_reset_blocked_owner_target`) · **ไม่รื้อ transport** (EventEmitter = test sink ที่ qa ขอ) · emit **post-commit เท่านั้น** | `architecture.md §9` · `§15 แถว 4` → `apps/api/src/auth/security-events.service.ts` | — | todo | — |
