...
| B-19 | **ปิดแล้วจริง** ผมตรวจโค้ด+เทสต์เอง (§5) · **AC ยังครบ 35/35** และ AC-4.2 หลักฐาน*แข็งขึ้น* (§4) |
...
B Design:      pass — architecture/data-model/api-spec/ux-wireframe/ui/test-plan ครบ + signoff:approved ·
                      security-review 3 ไฟล์ + advisory review ของ B-19 · AC→case ครบ 35/35 (§4)
...
Non-blocking ใหม่ในรอบนี้: §6 — 4 ข้อ (N-1..N-4) พร้อมเจ้าของ
...
|---|---|---|---|---|
| 17.1 | AC 35 ข้อมีเทสต์ที่รันจริงและผ่าน | ✅ | ✅ | §4 — ตรวจใหม่หลัง B-19 แล้ว ยัง 35/35 |
...
> **คำตอบ: ไม่มีข้อไหนเปลี่ยนสถานะ · ยังเป็น 35/35 (เต็ม 32 · partial 3) · แต่ AC-4.2 หลักฐาน "แข็งขึ้น" ไม่ใช่ "เพิ่งมี"**
...
**สรุป: 35/35 · เต็ม 32 · partial 3 (AC-1.2 · AC-5.4 · AC-7.3) — ตัวเลขเท่าเดิม**
...
### N-1 · หน้าต่าง 30 นาทีบนเครื่องที่ใช้ร่วมกัน — owner: **@frontend** (แจ้ง **@security-reviewer** ให้บันทึก)
...
### N-2 · `Referrer-Policy` ไม่มีใครพิสูจน์ว่า **เซิร์ฟเวอร์ส่งจริง** — owner: **@devops + @qa (ผมเอง)**
...
### N-3 · ปุ่ม "เชิญใหม่อีกครั้ง" = scope ที่ลงมาหลัง Gate 2 โดยไม่มี AC — owner: **@product (รับทราบ) + @qa**
...
| **R-3** | **มี commit ใหม่เข้า branch นี้** หลัง `65df986` | ตัวเลขทุกตัวใน §2 ผูกกับ headSha นี้ · CI run 35491463371 จะกลายเป็น run ของคอมมิตเก่าทันที | รัน CI ใหม่ + ผมตรวจ headSha ซ้ำ · ถ้าแตะ `/invite`/auth/สิทธิ์ ⇒ ตรวจ §4 ใหม่ด้วย |
