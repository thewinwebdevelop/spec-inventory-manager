...
| ② Decompose | หลัง G1✓ → วางชุด **เอกสาร Gate 2 ต่อทีม** (§2.2) + แปะ RACI + เปิด status · หลัง G2 sign-off → **แตก Task (§3.F)** | doc plan → task board (T-XXX) | ไม่ตัดสิน scope/AC เอง (→ product) |
| ③ Dispatch | Gate 2: dispatch แต่ละทีม gen เอกสาร · Build: จ่าย **T-ID** ให้ agent + skill — **เช็ค dependency ก่อน** | subagent task | dispatch เฉพาะงานที่ deps = done |
...
### 2.2 Gate 2 — Documentation flow (เอกสารต่อทีม + per-feature directory)
หลัง ✋ G1 sign-off → เข้า Gate 2: **แต่ละทีม gen เอกสารของตัวเอง** เก็บใน directory ของ feature
→ user sign off **ทีละฉบับ** → ครบแล้วจึงแตก task

**โครง directory ต่อ feature (feature ที่ถึง Gate 2):**
```
docs/features/F-021/
  F-021.md          # Gate 1: overview + user stories + AC        (product)
  architecture.md   # Gate 2: push/idempotency/retry (เฉพาะ full) (backend-api)
  data-model.md     # Gate 2: schema/migration                    (backend-api)
  api-spec.md       # Gate 2: OpenAPI contract                     (backend-api)
  ux-wireframe.md   # Gate 2: flow + wireframe + Thai copy         (ux)
  ui.md             # Gate 2: visual/token (reuse design system)   (ux)
  test-plan.md      # Gate 2: test cases จาก AC                    (qa)
  tasks.md          # Task board (T-XXX) — หลังเอกสาร sign-off ครบ  (PM)
```
> feature เล็ก (light) / Gate-1-only ยังเป็นไฟล์เดียว `F-XXX.md` ได้ — โปรโมตเป็น
> directory เมื่อเข้า Gate 2 แบบ full

**ลำดับ:**
1. PM dispatch แต่ละทีมให้ **gen เอกสารในตำแหน่งของตัวเอง** (ตาม dependency: architecture→data-model→api-spec→(ux ∥ test-plan))
2. **ระหว่างทำมีข้อสงสัย = ห้ามตัดสินเอง** → พ่น BLOCKED ถามทีมที่เกี่ยวข้อง (§3.C) → PM route → log D-XXX → ป้อนกลับ
3. ทีมทำเสร็จ → PM ส่งเอกสารให้ **user review + sign off ทีละฉบับ** (ผ่าน review เจ้าของส่วนก่อน)
4. **✋ sign off ครบทุกเอกสาร** = Gate 2 commit → commit เอกสารทั้ง directory
5. → **แตก task ทั้งหมดของทุกทีม (§3.F)** → จ่าย T-ID ให้ agent ตาม skill ของทีม → Build
...
| G2 doc (แต่ละทีม gen เอกสารตน) | C | R* | R* | R* | R* | C | – | drives | I |
| **G2 commit sign-off** ✋ (ต่อฉบับ) | C | C | C | C | C | I | I | drives | **A** |
...
> **R\*** (แถว G2 doc) = แต่ละทีม **R เฉพาะเอกสารของตัวเอง** (backend-api→architecture/data-model/api-spec · ux→wireframe/ui · qa→test-plan) — ไม่ใช่ทุกทีมเขียนทุกฉบับ
> user เป็น **A** ที่ human gate: G1 sign-off · **G2 sign-off (ทีละฉบับ)** · Release · Portfolio
...
**(1) แตก task — หลัง Gate 2 sign-off (เอกสารครบทุกทีม signed off, §2.2):**
- **PM นำการแตก** โดยให้แต่ละทีม (design/backend/frontend/qa + devops) เสนอ task ของตัวเอง **จากเอกสาร Gate 2 ที่ sign off แล้ว**
...
- แตกหลัง design นิ่ง → task ละเอียด/ครบตั้งแต่แรก · T-ID = ต่อ feature (`T-021-01`)
...
Task board format (`docs/features/F-021/tasks.md`) — **block ต่อ 1 task** (build task หลัง G2✓):
```md
# F-021 Task Board

## T-021-01 · backend · push logic
- desc: core-domain fn ตัดสต๊อก + emit push event เมื่อ available เปลี่ยน + unit test
- ref: architecture.md, data-model.md, api-spec.md
- deps: G2✓            · status: done         · updated_by: @backend-api

## T-021-02 · devops · sync queue
- desc: ตั้ง BullMQ queue + retry/backoff + env สำหรับ push job
- ref: architecture.md
- deps: G2✓            · status: in_progress  · updated_by: @devops

## T-021-03 · backend · sync-status endpoint
- desc: implement GET /channel-listings/{id}/sync-status ตาม contract
- ref: api-spec.md, data-model.md
- deps: T-021-01       · status: todo         · updated_by: —

## T-021-04 · frontend · sync status UI
- desc: หน้าจอสถานะ sync ต่อ listing — consume client + component จาก design
- ref: ux-wireframe.md, ui.md, api-spec.md
- deps: T-021-03       · status: blocked(dep) · updated_by: —

## T-021-05 · qa · E2E + verify AC
- desc: E2E flow + ตรวจทุก AC มี test + money/stock matrix
- ref: test-plan.md
- deps: T-021-04       · status: blocked(dep) · updated_by: —
```
**ฟิลด์ต่อ task:** `desc` (ทำอะไรคร่าวๆ) · `ref` (เอกสาร Gate-2 ที่ต้องอ้างอิง) · `deps` · `status` · `updated_by`
กติกา:
...
| `docs/features/_TEMPLATE.md` | เพิ่ม front-matter status/owner_now/waiting_on · Gate-1 เป็น `F-XXX.md` | แก้เล็ก |
| `docs/features/_gate2/` templates | ไฟล์ใหม่ — per-doc template: `architecture.md` · `data-model.md` · `api-spec.md` · `ux-wireframe.md` · `ui.md` · `test-plan.md` (§2.2) | สร้างใหม่ |
| `docs/features/_gate2/tasks.md` | ไฟล์ใหม่ — template task board (T-XXX + deps + status + updated_by) §3.F | สร้างใหม่ |
...
| `.claude/commands/gate1.md` · `gate2.md` | ไฟล์ใหม่ — slash command เดิน Gate 1 / Gate 2 (เรียก `/gate1`,`/gate2`) | สร้างใหม่ |
| `CLAUDE.md` | เพิ่มลิงก์ DECISIONS + SKILL_MAP + วิธีเรียก /gate1 /gate2 | แก้ |

> *ไม่แตะ:* `.claude/agents/*.md` ตัวอื่นรอบนี้ (follow-up ถ้าต้องการให้แต่ละ agent อ้าง skill ตัวเอง)
>
> **Bootstrap (สร้างนอกแผน):** `.claude/commands/setup-workflow.md` = command `/setup-workflow`
> ที่ execute แผนติดตั้งนี้ด้วย subagent-driven-development — สร้างนอกแผนเพราะมันคือ *ตัวรัน* แผน
> (สร้างด้วยแผนเองไม่ได้). ลำดับใช้งาน: `/setup-workflow` (ครั้งเดียว) → `/gate1` → `/gate2` ต่อ feature
