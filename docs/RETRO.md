...
### F-002 (ภาคสาม) · 2026-09-20 — ปิดฟีเจอร์ด้วย go-with-known-risk

> ภาคแรกคือ *เลนอัตโนมัติเจอสิ่งที่รีวิวไม่เจอ* · ภาคสองคือ *คนเดินแอปเจอสิ่งที่เลนไม่เจอ* ·
> ภาคนี้คือ **การปิดเองที่เกือบทำของพัง** — สามครั้งที่เกือบพลาด ไม่มีครั้งไหนที่ CI มองเห็นได้เลย

- **ช้าที่:** เลนเบราว์เซอร์แดงสองรอบติดโดย**ไม่ใช่ regression สักครั้ง** — รอบแรกผมรัน E-05 แยกไว้ก่อนจนกินโควตาล็อกอิน (20 ครั้ง/5 นาที) หมด · รอบสองผมลืมตั้ง `E2E_REDIS_URL` ⇒ `resetIpThrottle()` **no-op เงียบ ๆ** ทั้งที่ชื่อฟังก์ชันบอกว่ารีเซ็ตแล้ว · **วิธีรันที่ถูกเขียนไว้ครบใน `gate-e-verdict.md:81` มาตั้งแต่รอบก่อน — ผมไม่ได้เปิดอ่านก่อนรัน** · แล้ว Docker daemon ก็ตายกลางทาง ⇒ 4 เลน (Playwright · integration-api · db-migrate · mobile-e2e) เหลือหลักฐาน**ชั้นเดียวคือ CI** ซึ่ง qa บันทึกไว้ตรง ๆ แทนที่จะปล่อยให้เข้าใจว่าตรวจสองชั้น
- **กติกาที่ขาด:** **merge-base check มีอายุ** — `release` ตรวจไว้ 2026-09-05 ว่า merge-base ถูก (`25ae7f3`) แล้วถือว่าใช้ได้ตลอด · ระหว่างนั้น trunk ขยับไป **1 commit** จากการ **squash merge** PR #7 ⇒ งานชุดเดียวกันอยู่คนละรูปทรง git ⇒ **ชน 116 ไฟล์** ในวันที่จะปิดพอดี · และกฎเงื่อนไขโมฆะของ verdict เขียนว่า *"commit ใหม่บน branch นี้ = โมฆะ"* ซึ่ง**ทำให้ลายเซ็นรับความเสี่ยงฆ่าตัวเอง** (ลายเซ็นก็คือ commit ใหม่) — product ปฏิเสธที่จะตีความแทน qa และ qa แก้เป็นการจำแนกตาม*สิ่งที่ commit แตะ* (`git diff --name-only`) ไม่ใช่ตามข้อความ commit
- **Review จับพลาด:** security review ของ B-19 เจอว่า **`endSession` ไม่มีผู้เรียกในแอปเลย** ⇒ ปุ่มออกจากระบบไม่ได้จบ session state และ token ที่พักไว้ข้ามผู้ใช้คนถัดไปในแท็บเดียวกัน — **เทสต์ของผมเองรายงานว่าผ่าน** เพราะมันเรียก `endSession` ตรง ๆ จึงจริงเฉพาะฟังก์ชันที่ไม่มีใครใช้ (**`test-name-overclaims` รอบที่สอง ใน feature เดียวกัน**) · qa เจออีก 4 ข้อ (N-1..N-4) ทับบนทุกเลนเขียว · และ **merge เกือบทำของพังสองทาง ซึ่ง CI มองไม่เห็นทั้งคู่**: ปลุก `i18n/auth.ts` เวอร์ชันเก่าที่มี copy ซึ่ง M-02 เพิ่งแก้ทิ้ง และเกือบดึง `extractTraceId` กลับมา (finding ด้านความปลอดภัยที่ปิดไปแล้ว — คอมไพล์ผ่าน เทสต์เขียวหมด)
- **Dispatch/model:** ส่ง 3 ทีมขนานกันโดย**แบ่งตามไฟล์ที่ไม่ทับกัน** (ux=เอกสาร · devops=config · frontend=component) ได้ผลจริง ไม่มีใครเหยียบกัน · ส่งงานต่อให้ทีมเดิมด้วย context เดิมถูกกว่าเปิดใบใหม่ · **frontend แก้เทสต์ของผมแล้วเขียนเหตุผลกำกับ** (ต้อง hold หลัง mount ไม่งั้นกฎ route-guard ใหม่จะล้างให้ ⇒ ผ่านด้วยเหตุผลผิด) — นี่คือ agent ที่คิดต่อ ไม่ใช่ทำตามสั่ง · และ **agent ปฏิเสธการตีความแทนกัน 2 ครั้ง** (product ไม่ตีความ R-3 แทน qa · qa ไม่ยอมเปลี่ยน ❌ เป็น ✅ ให้ตัวเอง) = โครงสร้างความเป็นเจ้าของทำงานจริง
- **ทำซ้ำ/เลิกทำ:** **KEEP** — พิสูจน์ superset **ด้วยเครื่อง** ก่อน resolve: merge แล้ว diff เทียบ HEAD ก่อน merge ต้อง**ว่าง** ถ้าไม่ว่างแปลว่ามีของฝั่งโน้นที่เราไม่มี · **KEEP** — ตรวจ "ไฟล์หายเทียบ parent ทั้งสองฝั่ง" + กับดักที่ระบุชื่อมาก่อน (qa ชี้ `extractTraceId` ล่วงหน้า ทำให้ตรวจได้ตรงจุด) · **STOP** — รันเลนโดยไม่เปิดอ่านวิธีรันที่ทีมเขียนไว้แล้ว (เสียไปสองรอบเต็ม) · **STOP** — อ้าง CI ของ commit ก่อนหน้าแทน CI ของ commit ที่จะ merge จริง

> **Routing (learning loop):** (a) **"merge-base check มีอายุ"** — ต้องรันซ้ำ *ตอนจะ merge จริง* ไม่ใช่ครั้งเดียวตอนวางแผน → `release-gate-f.md §6` + memory `git-branch-workflow` · (b) **squash merge บน trunk + สาขาอายุยาว = ชนแน่นอน** และจำนวนไฟล์ที่ชนไม่ได้บอกว่าเนื้อหาขัดกัน → memory ใหม่ (`squash-merge-phantom-conflicts`) · (c) เงื่อนไขโมฆะของ verdict ต้องจำแนกตาม**ไฟล์ที่ commit แตะ** ไม่ใช่ "มี commit ใหม่ไหม" → เสนอเข้า **quality-gate Gate E** (Type 1 — รอ user เคาะ) · (d) `test-name-overclaims` **โผล่ซ้ำเป็นครั้งที่สองใน feature เดียวกัน** ในรูปใหม่ (เทสต์เรียกฟังก์ชันที่ production ไม่มีใครเรียก) → PM memory: ★ ทุกตัวต้องถามว่า *"เส้นทางที่เทสต์นี้เดิน มีคนเดินจริงในแอปไหม"* ซึ่งเป็น reachability ของ Gate D ในรูปของเทสต์
