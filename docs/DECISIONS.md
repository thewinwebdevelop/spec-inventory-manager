...
### D-025 · 2026-07-12 · cross-platform (architecture)

Q: รอบ "architecture deep-design" 2026-07-11 (PR #6) — รับ docs/architecture/* เป็น architecture authority ของทั้ง 3 platform + ยอมรับ hardening batch as-built ไหม?
Asked by: @claude (PM) Owner: @user
Decision: **[DRAFT — รอ user เคาะตอน review PR #6]** รับทั้งชุด:
**(1) docs/architecture/{mobile,web,backend}.md = architecture authority ต่อ platform** — feature ใหม่ implement ตาม pattern ในเอกสารนี้ ห้ามคิด pattern เอง (per-app CLAUDE.md ชี้ section ที่เกี่ยวแล้ว) · **(2) convention กลางล็อกร่วม 3 platform:** sealed `ApiFailure` taxonomy (Network/Throttled/AuthExpired/Forbidden/Entitlement/Validation/Conflict/NotFound/Server/ForceUpdate) map 1:1 กับ wire envelope `{error:{code,message,details?,fieldErrors?,traceId?}}` · cursor pagination `{items,nextCursor,total?}` · org ผ่าน `X-Organization-Id` header · gating 2 แกนแยกกัน (`can()` RBAC vs `entitled()` tier) · queue-backed งานหนัก = jobs resource (`202` + `GET /jobs/:id`) · **(3) ผล gap-analysis: ไม่มี REBUILD** — F-000/F-001 as-built เป็นฐานที่ถูก, gap = ชั้นที่ยังไม่สร้าง ผูก trigger ใน forward-commitments แล้ว · **(4) hardening batch as-built** (backend R1 filter+registry+gates · mobile R2 interceptor chain/R3 ApiFailure/R4 gen_l10n/R5 abstract provider + gate rules 6–7) รวม ★ sanity-pass fixes (generation-stamp fail-closed, traceId validation) — CI 8 jobs เขียวบน PR #6 · **(5) key platform choices ที่ล็อก:** mobile ยืน D-023 (Riverpod + feature-first 4 ชั้น) + interceptor chain · web = client-fetch-heavy (บังคับโดย auth transport D-019) + TanStack Query + org-in-URL `/o/[orgId]` + DataTable/Form pattern เดียว (build ตอน web-track restart, F-010) · backend = module map §2 + `ledgerWrite` primitive (F-011) + connector port + `ConnectorCapabilities` + DTO jail `mapping/vN/` (F-020) + `APP_ROLE` worker split-readiness
