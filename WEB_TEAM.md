...
- [ ] **เข้าถึงได้จริง (reachability)** — ทุกจอที่ feature อ้างว่า ship **ต้องมีเส้นทางจาก entry point ของแอปไปถึง และมีเทสต์เดินเส้นนั้น**
  · เช็คถูก ๆ ก่อน: `grep` ชื่อจอ/route ใน production tree — **0 อ้างอิงนอกไฟล์ตัวเอง = ไม่มีใครเปิดได้**
  · ⛔ เทสต์ที่ **สร้างจอขึ้นมาเอง** พิสูจน์ได้แค่ว่าจอทำงานเมื่อมีคนพามันขึ้นมา ไม่เคยพิสูจน์ว่ามีใครไปถึง
  · ⛔ **ของที่หายไปไม่ render ไม่ assert ไม่ throw** — เครื่องมือชนิด "รันแล้วดูว่าพังไหม" มองไม่เห็นโดยหลักการ ต้องเทียบกับสเปกว่าอะไรควรมี
  · ที่มา: F-002 · **B-14** (`/` เป็น placeholder เพราะไม่มีเทสต์ไหนเปิดมัน) · **B-16** (ทั้งแอปไม่มีปุ่มออกจากระบบ) · **B-18** (จอมือถือ 4 จอมี 0 อ้างอิง ขณะที่ CHANGELOG ประกาศว่า ship แล้ว) — ทั้งสามเจอโดยคนเปิดแอปด้วยมือ ไม่ใช่โดยเลนไหน
