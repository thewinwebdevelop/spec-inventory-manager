...
// Also emit a PURE TypeScript data module that core-domain imports at runtime.
// core-domain is framework/fs-free (golden rule #6) — it must NOT read the .txt
// via node:fs at module load (that breaks the clean tsc build, which has no Node
// types). The .txt above stays the human-diffable, SHA-pinned source of truth;
// this generated module is derived byte-for-byte from the same `body`, so the
// two can never drift (verify:fixture asserts it). JSON.stringify → a fully
// escaped, safe string literal regardless of fixture contents.
const MODULE_OUT = join(__dirname, "..", "src", "auth", "common-passwords.generated.ts");
const moduleBody =
  "// GENERATED by scripts/gen-breached-fixture.mjs — DO NOT EDIT BY HAND.\n" +
  "// Source of truth: fixtures/common-passwords-top10k.txt (SHA-256 pinned in\n" +
  "// fixtures/SOURCE.md). Regenerate with `pnpm --filter @omnistock/core-domain\n" +
  "// gen:fixture`. Committed as a pure data module so core-domain stays\n" +
  "// framework/fs-free (golden rule #6) and builds with no Node types.\n" +
  "/* eslint-disable */\n" +
  `export const COMMON_PASSWORDS_RAW = ${JSON.stringify(body)};\n`;
writeFileSync(MODULE_OUT, moduleBody, "utf8");
