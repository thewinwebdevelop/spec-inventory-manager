...
>
> **✅ เวอร์ชันตรงกันแล้ว (ตัดสิน 2026-09-05, `release` — ดู `docs/features/F-002/release-gate-f.md` §1):**
> ทุก workspace ของ npm = `0.0.0` และ `apps/mobile/pubspec.yaml` = `0.0.0+1` (เดิมเป็น `1.0.0+1`
> ค่า default ของ `flutter create` ที่ไม่มีใครแก้ — อ้างเวอร์ชันที่ไม่เคย ship) · build number `+1`
> คงไว้เพราะสโตร์บังคับให้เป็นจำนวนเต็มที่เดินหน้าอย่างเดียว คนละคำถามกับเลขหน้า `0.0.0`
> **นี่ไม่ใช่การตัดเวอร์ชันแรก** — `release` ยังเป็นคนตัด `0.1.0` (หรืออะไรก็ตาม) ตอนปิด Phase 1
