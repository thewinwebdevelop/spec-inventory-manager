...
- `/invite` ทำ **token ของคำเชิญหายระหว่างไปสมัคร/ล็อกอิน** แล้วขึ้นว่า "ลิงก์ใช้ไม่ได้" ทั้งที่ลิงก์ยังดี
  (B-19 · เจอตอนเดิน M-01 บนมือถือจริง) · ตอนนี้ token เดินทางไปกับผู้ใช้ **ในหน่วยความจำ** ตาม ux-wireframe §11.1
  แล้วพากลับมาที่คำเชิญเอง — ไม่เขียนลง URL/storage/cookie (E-12 ยังครบทุกข้อ)
- ปุ่ม **"ออกจากระบบ"** ใน sidebar ไม่เคยเรียก `endSession` เลย ⇒ ระบบยังถือว่า "ล็อกอินอยู่" หลังออกจากระบบ
  และคำเชิญที่พักไว้ค้างข้ามผู้ใช้คนถัดไปในแท็บเดียวกัน (เจอโดย security review ของ B-19)
- เปิด `/invite` โดยไม่มีลิงก์ติดมือ (รีเฟรช/แท็บใหม่ระหว่างสมัคร) เคยใช้ถ้อยคำของ `INVITATION_INVALID`
  ซึ่งไม่จริงสักข้อ · ตอนนี้เป็นสถานะของตัวเอง (`NO_TOKEN`, ux-wireframe §11.5) ที่บอกทางแก้ที่ได้ผลจริง
- แถวคำเชิญที่ **หมดอายุ/ยกเลิก** ไม่มีทางเชิญซ้ำ — เพิ่มปุ่ม "เชิญใหม่อีกครั้ง" ตาม ux-wireframe §7
  (ไม่ขึ้นบนคำเชิญที่รับไปแล้ว เพราะคนนั้นอยู่ในร้านแล้ว)
